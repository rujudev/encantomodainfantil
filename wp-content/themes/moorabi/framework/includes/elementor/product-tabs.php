<?php
if (!class_exists('Moorabi_Elementor_Product_Tabs')) {
    class Moorabi_Elementor_Product_Tabs extends Moorabi_Elementor
    {
        public $name = 'product-tabs';
        public $title = 'Product Tabs';
        public $icon = 'moorabi-elementor-icon eicon-product-tabs';
        ...
        protected function register_controls()
        {
            ...
            $this->add_control(
                'ajax_check',
                [
                    'label' => esc_html__('Using Ajax Tabs', 'moorabi'),
                    'type' => \Elementor\Controls_Manager::SELECT,
                    'options' => [
                        '1' => esc_html__('Yes', 'moorabi'),
                        '0' => esc_html__('No', 'moorabi'),
                    ],
                    'default' => '0',
                    'label_block' => true
                ]
            ...
            $this->add_control(
                'css_animation',
                [
                    'label' => esc_html__('Css Animation', 'moorabi'),
            ...
            $this->start_controls_section(
                'product_section',
                [
                    'label' => esc_html__('Product', 'moorabi'),
                ]
            );
            $this->add_control(
                'style',
                [
                    'label' => esc_html__('Style', 'moorabi'),
                    'type' => \Elementor\Controls_Manager::SELECT,
                    'options' => Moorabi_Elementor::moorabi_elementor_preview('products'),
                    'default' => 'style-01',
                    'label_block' => true
                ]
            );
            foreach (Moorabi_Elementor::moorabi_elementor_preview('products') as $key => $value) {
                $this->add_control(
                    $key,
                    [
                        'label' => '<img alt="' . esc_attr($this->name) . '" src="' . esc_url(get_theme_file_uri('/assets/images/elementor/products\/' . $key . '.jpg')) . '"/>',
                        'type' => \Elementor\Controls_Manager::HEADING,
                        'condition' => array(
                            'style' => $key
                        ),
            ...
            $repeater->add_control(
                'title', [
                    'label' => esc_html__('Tab Title', 'moorabi'),
                    'type' => \Elementor\Controls_Manager::TEXT,
                    'placeholder' => esc_html__('Enter your tab title', 'moorabi'),
                    'default' => esc_html__('Tab', 'moorabi'),
                    'label_block' => true
                ]
            );
            $repeater->add_control(
            ...
            $repeater->add_control(
                ...
                [
                    ...
                    'condition' => array(
                        'target' => array('top-rated', 'recent-product', 'product-category', 'featured_products', 'on_sale', 'on_new', 'product_attribute')
                    ),
                    'label_block' => true
        ...
        protected function render()
        {
            ...
            $css_class = array('moorabi-tabs');
            $css_class[] = $atts['styletab'];
            ...
            $list_class = array('response-product');
            $owl_settings = '';
            if ($atts['layout'] == 'grid') {
                $list_class[] = 'product-list-grid row auto-clear equal-container better-height ';
            ...
            if ($atts['layout'] == 'carousel') {
                $list_class[] = 'product-list-owl owl-slick equal-container better-height';
            ...
            <div class="<?php echo esc_attr(implode(' ', $css_class)); ?>">
                <?php if ($atts['list']) : ?>
                    <div class="tab-head">
                        <ul class="tab-link equal-container">
                            <?php
                            foreach ($atts['list'] as $key => $value) :
                                $class_active = '';
                                $class_loaded = '';
                                if ($key == 0) {
                                    $class_active = 'active';
                                ...
                                <li class="<?php echo esc_attr($class_active); ?>">
                                    <a class="<?php echo esc_attr($class_loaded); ?>"
                                       ...
                                       href="#<?php echo esc_attr($key); ?>-<?php echo esc_attr($rand); ?>">
                    ...
                    <div class="tab-container">
                        <?php foreach ($atts['list'] as $key => $value):
                            $products = apply_filters('moorabi_get_products', $value);
                            $id_loop = array();
                            ?>
                            <div class="tab-panel <?php if ($key == 0): ?>active<?php endif; ?>"
                                 id="<?php echo esc_attr($key); ?>-<?php echo esc_attr($rand); ?>">
                                        ...
                                        <?php if ($products->have_posts()): ?>
                                            <div
                                                ...
                                                <?php while ($products->have_posts()) : $products->the_post(); ?>
                                                    ...
                                                    <div <?php post_class(array_merge($item_class, array($value['target']))); ?>>
