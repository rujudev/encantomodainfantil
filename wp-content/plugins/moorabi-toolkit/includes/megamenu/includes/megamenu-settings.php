<?php
/**
 * Moorabi Megamenu Settings
 *
 * @author
 * @category
 * @package  Moorabi_Megamenu_Settings
 * @since    1.0.0
 */
if (!defined('ABSPATH')) {
    exit; // Exit if accessed directly.
}
if (!class_exists('Moorabi_Megamenu_Settings')) {
    class Moorabi_Megamenu_Settings extends Moorabi_Megamenu
    {
        public static $index = 0;

        public function __construct()
        {
            add_action('wp_ajax_moorabi_get_form_settings', array($this, 'ajax_form_settings'));
            add_action('wp_ajax_moorabi_save_all_settings', array($this, 'save_all_settings'));
            add_action('wp_ajax_moorabi_create_mega_menu', array($this, 'create_mega_menu'));
            add_action('wp_ajax_moorabi_remove_mega_menu', array($this, 'remove_mega_menu'));
            add_action('wp_ajax_moorabi_button_settings', array($this, 'button_settings'));
            /* EDIT WALKER */
            ...
            add_action('after_setup_theme', array($this, '_theme_setup'), 11);
            add_action('wp_footer', array($this, '_clone_mobile_menu'));
            /* MOBILE MENU AJAX */
            add_action('wp_ajax_moorabi_load_mobile_menu', array($this, 'load_mobile_menu'));
            add_action('wp_ajax_nopriv_moorabi_load_mobile_menu', array($this, 'load_mobile_menu'));

            /* MEGAMENU MENU AJAX */
            add_action('wp_ajax_moorabi_load_mega_menu', array($this, 'load_mega_menu'));
            add_action('wp_ajax_nopriv_moorabi_load_mega_menu', array($this, 'load_mega_menu'));
        }
        ...
        public function _theme_setup()
        {
            register_nav_menus(array(
                    'moorabi_mobile_menu' => esc_html__('Mobile Menu', 'moorabi-addon-toolkit'),
                )
            );
        }
        ...
        public function _add_content()
        {
            if ($GLOBALS['pagenow'] == 'nav-menus.php') {
                include_once MOORABI_MEGAMENU_DIR.'templates/form-settings.php';
            }
        }

        public static function is_enable_megamenu($settings, $depth = 0)
        {
            if (MOORABI_TOOLKIT()->is_vc_editor() || MOORABI_TOOLKIT()->is_elementor_editor()) {
                return false;
            }
        ...
        public static function _return_menu_items($menu_locations, $default)
        {
                ...
                foreach ($menu_locations as $location) {
                    ...
                    if (!empty($menu_items)) {
                        ...
                        foreach ($menu_items as $key => $menu_item) {
                            ...
                            $settings  = get_post_meta($menu_item->ID, '_moorabi_menu_settings', true);
                            /* CHECK MEGA MENU */
            ...
            return apply_filters('moorabi_data_mobile_menu_locations',
                array(
                    'menus' => $array_menus,
                    'child' => $array_child,
                )
        ...
        public static function _mobile_menu_panels($menu_locations, $default)
        {
            ...
            if (!empty($data['menus'])):
                ...
                foreach ($data['menus'] as $parent_id => $menus) :
                    ...
                    if ($count == 0) {
                        echo "<div id='moorabi-menu-panel-{$main_id}' class='moorabi-menu-panel moorabi-menu-panel-main'>";
                    } else {
                        echo "<div id='moorabi-menu-panel-{$parent_id}' class='moorabi-menu-panel moorabi-menu-sub-panel moorabi-menu-hidden'>";
                    }
                    ...
                    foreach ($menus as $id => $menu) {
                        ...
                        if ($menu['mega'] == true) {
                            ...
                            if (in_array($id, $data['child'])) {
                                echo "<a class='moorabi-menu-next-panel' href='#moorabi-menu-panel-{$id}'></a>";
                            }
        ...
        {
            check_ajax_referer('moorabi_ajax_megamenu', 'security');
        ...
        public static function load_mega_menu()
        {
            ...
            if (MOORABI_TOOLKIT()->is_elementor($megamenu_id)) {
                $with_css = MOORABI_TOOLKIT()->is_request('ajax');
        ...
        public static function sample_mobile_menu($menu_locations, $default = 'primary')
        {
            ...
            if (!empty($menu_locations)) {
                ...
                $mobile_menu .= "<span class='moorabi-menu-current-panel-title'>".esc_html__('Main Menu',
                        'moorabi-addon-toolkit')."</span>";
                $mobile_menu .= "<a href='#' class='moorabi-menu-close-btn moorabi-menu-close-panels'>x</a>";
                $mobile_menu .= "</div>";
                ...
                foreach ($array_menus as $parent_id => $menus) {
                    ...
                    foreach ($menus as $id => $menu) {
                        ...
                        if (in_array($id, $array_child)) {
                            $mobile_menu .= "<a class='moorabi-menu-next-panel' href='#moorabi-menu-panel-{$id}'></a>";
                        }
        ...
        public static function install_mobile_menu($menu_locations, $args = array())
        {
            if (!empty($menu_locations)) :
                ...
                $default = array(
                    ...
                    'title'   => esc_html__('Main Menu', 'moorabi-addon-toolkit'),
                );
                $args    = wp_parse_args($args, $default);
                $classes = array('moorabi-menu-clone-wrap', $args['class']);
                ...
                <div id="moorabi-menu-mobile-<?php echo esc_attr(self::$index); ?>"
                     class="<?php echo esc_attr(implode(' ', $classes)); ?>"
                     data-locations="<?php echo esc_attr(json_encode($menu_locations)); ?>"
                     data-default="<?php echo esc_attr($args['default']); ?>">

                    <?php do_action('moorabi_before_html_mobile_menu', $menu_locations, $args); ?>

                    <div class="moorabi-menu-panels-actions-wrap">

                        <span class="moorabi-menu-current-panel-title"
                              data-main-title="<?php echo esc_attr($args['title']); ?>">
                            <?php echo esc_html($args['title']); ?>
                        </span>

                        <a href="#" class="moorabi-menu-close-btn moorabi-menu-close-panels">x</a>

                        <?php do_action('moorabi_title_html_mobile_menu', $menu_locations, $args); ?>

                    </div><!-- .moorabi-menu-panels-actions-wrap -->

                    <?php do_action('moorabi_before_panels_mobile_menu', $menu_locations, $args); ?>
                    ...
                    </div><!-- .moorabi-menu-panels -->

                    <?php do_action('moorabi_after_html_mobile_menu', $menu_locations, $args); ?>

                </div><!-- .moorabi-menu-clone-wrap -->
        ...
        public function _clone_mobile_menu()
        {
            ...
            if (!empty($locations['moorabi_mobile_menu'])) {
                $mobile_menu = wp_get_nav_menu_object($locations['moorabi_mobile_menu']);
                $menus[]     = $mobile_menu->slug;
            }

            $menus  = apply_filters('moorabi_menu_locations_mobile', $menus, $locations);

            self::install_mobile_menu($menus);

            do_action('moorabi_install_mobile_menu', $menus, $locations);
        ...
        public static function _return_title_menu_item($menu_item, $settings)
        {
            ...
            return apply_filters('moorabi_title_menu_item', $title, $menu_item, $settings);
        }
        ...
        public static function _return_megamenu_item($menu_item, $settings, $depth = 0)
        {
            ...
            if ($enable_mega == true) {
                ...
                if ($menu_content_id > 0) {
                    ...
                    $_output .= '>';
                    if (MOORABI_TOOLKIT()->is_elementor($menu_content_id)) {
                        $with_css = MOORABI_TOOLKIT()->is_request('ajax');
            ...
            return apply_filters('moorabi_content_megamenu_item', $_output, $menu_item, $settings);
        }
        ...
        public function _change_nav_menu_args($args)
        {
            ...
            $megamenu_layout         = 'moorabi-menu-wapper '.$megamenu_layout;
            $args['container_class'] = !empty($args['container_class']) ? $args['container_class'].' '.$megamenu_layout : $megamenu_layout;
            $args['menu_class']      = !empty($args['menu_class']) ? $args['menu_class'].' moorabi-menu' : 'moorabi-menu';
            if (isset($args['mobile_enable']) && $args['mobile_enable'] == 1) {
                $args['menu_class']      .= ' moorabi-clone-mobile-menu';
        ...
        public function save_all_settings()
        {
            ...
            if (!$item_id) {
                $response['errors'][] = esc_html__('Menu item not exists.', 'moorabi-addon-toolkit');
