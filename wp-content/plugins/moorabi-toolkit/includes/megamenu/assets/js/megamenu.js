...
(function ($) {
    ...
    $.fn.moorabi_resize_megamenu = function () {

        var megamenu = $(this);

        megamenu.on('moorabi_resize_megamenu', function () {
            ...
            if ($(this).length > 0 && window_size > 991) {
                $(this).each(function () {
                    var _class_responsive = $(this).children('.megamenu').data('responsive'),
                        _container        = $(this).closest('.moorabi-menu-wapper');
        ...
        }).trigger('moorabi_resize_megamenu');

        $(window).on('resize', function () {
            megamenu.trigger('moorabi_resize_megamenu');
        });
    ...
    /**==============================
     ...
     ===============================**/
    $.fn.moorabi_vertical_megamenu = function () {

        var vertical_menu = $(this);

        vertical_menu.on('moorabi_vertical_megamenu', function () {
            $(this).each(function () {
                var menu        = $(this),
        ...
        }).trigger('moorabi_vertical_megamenu');

        $(window).on('resize', function () {
            vertical_menu.trigger('moorabi_vertical_megamenu');
        });
    ...
    function load_mobile_menu($menu) {
        if (!$menu.hasClass('loaded')) {
            $.ajax({
                type    : 'POST',
                url     : moorabi_ajax_megamenu.ajaxurl,
                data    : {
                    action   : 'moorabi_load_mobile_menu',
                    security : moorabi_ajax_megamenu.security,
                    locations: $menu.data('locations'),
                    default  : $menu.data('default'),
                },
                success : function (response) {
                    if (response.success == true) {
                        $menu.children('.moorabi-menu-panels').html(response.data);
                    }
                },
    ...
    if (moorabi_ajax_megamenu.load_megamenu == true) {
        $('.moorabi-menu-wapper .menu-item.item-megamenu').hover(function () {
            var $menu     = $(this),
    ...
    $(document).on('click', '.menu-toggle', function () {
        var $button = $(this),
            ...
            $menu   = $('.moorabi-menu-clone-wrap');

        if ($index != undefined && $('#moorabi-menu-mobile-' + $index).length) {
            $menu = $('#moorabi-menu-mobile-' + $index);
        }

        $('body').addClass('moorabi-open-mobile-menu');
        $menu.addClass('open');

        if (moorabi_ajax_megamenu.load_menu == 'click') {
    ...
    $(document).on('click', '.moorabi-menu-clone-wrap .moorabi-menu-close-panels', function () {
        $('body').removeClass('moorabi-open-mobile-menu');
        $('.moorabi-menu-clone-wrap').removeClass('open');
        return false;
    });
    $(document).on('click', function (event) {
        var menu_mobile = $('.moorabi-menu-clone-wrap');
    ...
    $(document).on('click', '.moorabi-menu-next-panel', function (e) {
        var thisButton   = $(this),
            ...
            thisPanel    = thisButton.closest('.moorabi-menu-panel'),
            thisMenu     = thisButton.closest('.moorabi-menu-clone-wrap'),
            currentTitle = thisMenu.find('.moorabi-menu-current-panel-title'),
            actionsWrap  = thisMenu.find('.moorabi-menu-panels-actions-wrap'),
        ...
        if (targetElem.length) {
            ...
            var itemTitle      = thisItem.children('.menu-link').html(),
                prevPanel      = $('<a class="moorabi-menu-prev-panel"></a>'),
                firstItemTitle = '';

            thisPanel.addClass('moorabi-menu-sub-opened');
            targetElem.removeClass('moorabi-menu-hidden').addClass('moorabi-menu-panel-opened').attr('data-parent-title', itemTitle).attr('data-parent-panel', thisPanel.attr('id'));
            ...
            if (typeof itemTitle != 'undefined' && typeof itemTitle !== false) {
                if (!currentTitle.length) {
                    actionsWrap.prepend('<span class="moorabi-menu-current-panel-title"></span>');
                }
            ...
            actionsWrap.find('.moorabi-menu-prev-panel').remove();
    ...
    $(document).on('click', '.moorabi-menu-prev-panel', function (e) {
        ...
        thisMenu.find(currentPanel).removeClass('moorabi-menu-panel-opened').addClass('moorabi-menu-hidden');
        targetElem.addClass('moorabi-menu-panel-opened').removeClass('moorabi-menu-sub-opened');
    ...
    $(document).on('click', '.moorabi-menu-clone-wrap .menu-item.disable-link > .menu-link', function (e) {
        $(this).prev('.moorabi-menu-next-panel').trigger('click');
        e.preventDefault();
    });
    ...
    $(document).ready(function () {
        ...
        if (moorabi_ajax_megamenu.resize == 'mobile' && Mobile_Detect.Mobile()) {
            enable_resize = false;
        }

        if (moorabi_ajax_megamenu.resize == 'tablet' && Mobile_Detect.any()) {
            enable_resize = false;
        }

        if (enable_resize) {

            var horizontal = $('.moorabi-menu-wapper.horizontal'),
                vertical   = $('.moorabi-menu-wapper.vertical');

            if (horizontal.length) {
                horizontal.each(function () {
                    if ($(this).find('.item-megamenu').length) {
                        $(this).find('.item-megamenu').moorabi_resize_megamenu();
                    }
                });
            }
            if (vertical.length) {
                vertical.moorabi_vertical_megamenu();
            }
    ...
    if (moorabi_ajax_megamenu.load_menu == 'last') {
        ...
        window.addEventListener("load", function load() {
            ...
            setTimeout(function () {
                $('.moorabi-menu-clone-wrap').each(function () {
                    load_mobile_menu($(this));
                });
            }, moorabi_ajax_megamenu.delay);
