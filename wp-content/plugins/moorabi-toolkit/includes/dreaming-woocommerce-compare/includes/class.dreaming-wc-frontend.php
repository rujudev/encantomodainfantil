...
if ( ! class_exists( 'Dreaming_Woocompare_Frontend' ) ) {
	class Dreaming_Woocompare_Frontend {
		 ...
		 * @since 1.0.0
		 */
		public $cookie_name = 'dreaming_wc_compare_list';
		 ...
		 * @since 1.0.0
		 */
		public $action_add = 'dreaming_wccp_add_product';
		 ...
		 * @since 1.0.0
		 */
		public $action_remove = 'dreaming_wccp_remove_product';
		 ...
		 * @since 1.0.0
		 */
		public $action_reload = 'dreaming_wccp_reload_product';
		 ...
		 * @since 1.0.0
		 */
		public $action_view = 'dreaming_wccp_view_compare_page';
		 ...
		 * Constructor
		 *
		 * @return Dreaming_Woocompare_Frontend
		 * @since 1.0.0
		 */
		public function __construct() {
			...
			add_action( 'wp_ajax_dreaming_wccp_search_product_via_ajax', array( $this, 'search_product_via_ajax' ) );
			...
			add_action( 'wp_ajax_nopriv_dreaming_wccp_search_product_via_ajax', array( $this, 'search_product_via_ajax' ) );
		...
		public function init_params() {
			...
			$lang = isset( $_REQUEST['lang'] ) ? Dreaming_Woocompare_Helper::clean( $_REQUEST['lang'] ) : false;
			...
			$this->default_fields = Dreaming_Woocompare_Helper::default_selected_compare_fields();
		...
		public function frontend_scripts( $hook ) {
			...
			wp_enqueue_style( 'slick', MOORABI_TOOLKIT_URL . 'includes/dreaming-woocommerce-compare/assets/vendors/slick/slick.min.css' );

			wp_enqueue_style( 'dreaming-wccp-frontend', MOORABI_TOOLKIT_URL . 'includes/dreaming-woocommerce-compare/assets/css/frontend.css' );
			
			wp_enqueue_script( 'slick', MOORABI_TOOLKIT_URL . 'includes/dreaming-woocommerce-compare/assets/vendors/slick/slick.min.js', array( 'jquery' ), null );
				
			wp_enqueue_script( 'dreaming-wccp-frontend', MOORABI_TOOLKIT_URL . 'includes/dreaming-woocommerce-compare/assets/js/frontend.js', array( 'jquery' ), null );
			
			ob_start();
			Dreaming_Woocompare_Helper::get_template_part( 'compare', 'products-list' );
			$products_list_tmp = ob_get_clean();
			
			ob_start();
			Dreaming_Woocompare_Helper::get_template_part( 'add-products', 'form' );
			$add_product_form = ob_get_clean();
			
			$dreaming_wccp_args = array(
				'ajaxurl'      => admin_url( 'admin-ajax.php' ),
				'security'     => wp_create_nonce( 'dreaming_wccp_nonce' ),
				'ajax_actions' => $this->action_names,
				'text'         => array(
					'added'   => esc_html__( 'Added to compare', 'moorabi-toolkit' ),
					'compare' => esc_html__( 'Compare', 'moorabi-toolkit' )
				),
				'template'     => array(
			...
			wp_localize_script( 'dreaming-wccp-frontend', 'dreaming_wccp', $dreaming_wccp_args );
		}
		...
		public function template_redirect( $original_template ) {
			if ( is_page() ) {
				...
				if ( $page_id == $page_for_compare ) {
					return Dreaming_Woocompare_Helper::locate_template( 'compare.php' );
				}
			}
		...
		public function search_product_via_ajax() {
			...
			if ( ! isset( $_REQUEST['search_keyword'] ) || ! isset( $_REQUEST['action'] ) || $_REQUEST['action'] != 'dreaming_wccp_search_product_via_ajax' ) {
				wp_send_json( $response );
				die();
			}
			
			$search_keyword = Dreaming_Woocompare_Helper::clean( $_REQUEST['search_keyword'] );
			...
			if ( $products ) {
				$img_w = 90;
				$img_h = 90;
				if ( ! has_image_size( 'dreaming_img_size_90x90' ) ) {
					$img_w = 150;
					$img_h = 150;
				}
				foreach ( $products as $product ) {
					$product_id       = intval( $product->get_id() );
					$thumb            = Dreaming_Woocompare_Helper::resize_image( get_post_thumbnail_id( $product_id ), null, $img_w, $img_h, true, true, false );
					$response['html'] .= '<a href="' . esc_url( get_permalink( $product_id ) ) . '" title="' . esc_attr__( 'Add to compare', 'moorabi-toolkit' ) . '" class="dreaming-wccp-search-result-item" data-product_id="' . $product_id . '"><img width="' . esc_attr( $thumb['width'] ) . '" height="' . esc_attr( $thumb['height'] ) . '" src="' . esc_url( $thumb['url'] ) . '" /> <span class="product-name">' . $product->get_name() . '</span></a>';
		...
		public function add_product_to_compare_ajax() {
			...
			$product_id     = intval( $_REQUEST['id'] );
			$include_return = Dreaming_Woocompare_Helper::clean( $_REQUEST['include_return'] );
			$product        = $this->wc_get_product( $product_id );
		...
		public function remove_product_from_compare_ajax() {
			...
			$response_type = isset( $_REQUEST['response_type'] ) ? Dreaming_Woocompare_Helper::clean( $_REQUEST['response_type'] ) : '';
		...
		public function get_products_list( $products = array() ) {
			...
			$fields = Dreaming_Woocompare_Helper::get_selected_compare_fields_with_texts();
				...
				foreach ( $fields as $field => $name ) {
					...
					switch ( $field ) {
						...
						case 'description':
							...
							$product->fields[ $field ] = apply_filters( 'dreaming_wccp_woocompare_products_description', $description );
							break;
						case 'stock':
							$availability = $product->get_availability();
							if ( empty( $availability['availability'] ) ) {
								$availability['availability'] = esc_html__( 'In stock', 'moorabi-toolkit' );
							}
		...
		public function list_products_html( $lang = false ) {
			...
			if ( empty( $this->products_list ) ) {
				$html .= '<div class="list_empty">' . esc_html__( 'No products to compare', 'moorabi-toolkit' ) . '</div>';
			} else {
				foreach ( $this->products_list as $product_id ) {
					...
					$thumb_hover = Dreaming_Woocompare_Helper::resize_image( get_post_thumbnail_id( $product_id ), 'null', $all_settings['compare_img_size_w'], $all_settings['compare_img_size_h'], true, true, false );
					$thumb       = Dreaming_Woocompare_Helper::resize_image( get_post_thumbnail_id( $product_id ), 'null', $all_settings['panel_img_size_w'], $all_settings['panel_img_size_h'], true, true, false );
					$html        .= '<div class="compare-item">
	                            ...
	                            <a href="' . esc_url( $this->remove_product_url( $product_id ) ) . '"
		                           ...
		                           title="' . esc_attr__( 'Remove', 'moorabi-toolkit' ) . '">x</a>
		                    </div>';
				}
			}
		...
		public function add_compare_link( $product_id = 0, $args = array() ) {
			...
			printf( '<a href="%s" class="%s" data-product_id="%d" data-product_url="%s" rel="nofollow">%s</a>', $this->add_product_url( $product_id ), 'dreaming-wccp-button', $product_id, $this->add_product_url( $product_id ), esc_html__( 'Add to compare', 'moorabi-toolkit' ) );
		}
		...
		public function add_product_url( $product_id ) {
			...
			return apply_filters( 'dreaming_wccp_add_product_url', esc_url_raw( add_query_arg( $url_args, site_url() ) ), $this->action_add );
		}
		...
		public function compare_page_url() {
			...
			$page_url = Dreaming_Woocompare_Helper::get_page_link( 'compare' );
			$url_args = array();
			$lang     = defined( 'ICL_LANGUAGE_CODE' ) ? ICL_LANGUAGE_CODE : false;
			if ( $lang ) {
				$url_args['lang'] = $lang;
		...
		public function compare_table_html() {
			$html = '';
			ob_start();
			Dreaming_Woocompare_Helper::get_template( 'compare-table.php', array( 'products_list' => $this->products_list ) );
			$html .= ob_get_clean();
