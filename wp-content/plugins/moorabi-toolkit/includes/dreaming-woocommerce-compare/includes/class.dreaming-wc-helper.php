 ...
 *
 * @author  Moorabi
 * @package Dreaming WooCommerce Compare
 * @version 1.0.0
 */
...
if ( ! class_exists( 'Dreaming_Woocompare_Helper' ) ) {
	
	/**
	 *
	 * @since 1.0.0
	 */
	class Dreaming_Woocompare_Helper {
		...
		public static function get_all_settings() {
			...
			$all_settings        = get_option( 'dreaming_wccp_all_settings', array() );
		...
		public static function set_image_size() {
			...
			add_image_size( 'dreaming_wccp_image', $size['width'], $size['height'], $size['crop'] );
		}
		...
		public static function compare_fields() {
			...
			$fields = array(
                ...
                'add-to-cart' => esc_html__( 'Add to cart', 'moorabi-toolkit' ),
				'title'       => esc_html__( 'Title', 'moorabi-toolkit' ),
				'price'       => esc_html__( 'Price', 'moorabi-toolkit' ),
				'sku'         => esc_html__( 'Sku', 'moorabi-toolkit' ),
				'stock'       => esc_html__( 'Availability', 'moorabi-toolkit' ),
				'weight'      => esc_html__( 'Weight', 'moorabi-toolkit' ),
				'dimensions'  => esc_html__( 'Dimensions', 'moorabi-toolkit' ),
                'description' => esc_html__( 'Description', 'moorabi-toolkit' ),
			);
			
			$fields = array_merge( $fields, Dreaming_Woocompare_Helper::attribute_taxonomies() );
		...
		public static function all_single_product_hooks_select_html( $selected = 'woocommerce_after_add_to_cart_form', $class = '', $name = '', $id = '', $echo = true ) {
			...
			$html = '<option value="dreaming_wccp_single_product">' . esc_html__( 'Custom hook (dreaming_wccp_single_product)', 'moorabi-toolkit' ) . '</option>';
			...
			$html_atts .= 'class="dreaming-wccp-select dreaming-wccp-single-product-hook-select ' . esc_attr( $class ) . '" ';
		...
		public static function all_products_list_hooks_select_html( $selected = 'dreaming_wccp_shop_loop', $class = '', $name = '', $id = '', $echo = true ) {
			$all_single_hooks = array(
			...
			$html = '<option value="dreaming_wccp_shop_loop">' . esc_html__( 'Custom hook (dreaming_wccp_shop_loop)', 'moorabi-toolkit' ) . '</option>';
			...
			$html_atts .= 'class="dreaming-wccp-select dreaming-wccp-products-list-hook-select ' . esc_attr( $class ) . '" ';
		...
		public static function all_pages_select_html( $selected = '', $class = '', $name = '', $id = '', $exception = array(), $echo = true ) {
			...
			$html = '<option value="">' . esc_html__( 'Select Page', 'moorabi-toolkit' ) . '</option>';
			...
			$html_atts .= 'class="dreaming-wccp-select dreaming-wccp-page-select ' . esc_attr( $class ) . '" ';
		...
		public static function compare_admin_fields_cb_html() {
			...
			if ( $fields ) {
				foreach ( $fields as $field_key => $field_val ) {
					...
					$html .= '<li><label><input type="checkbox" ' . checked( in_array( $field_key, $selected_fields ), true, false ) . ' class="compare-field-cb" name="dreaming_wccp_fields_attrs[]" id="dreaming_wccp_fields_attrs_' . esc_attr( $field_key ) . '" value="' . esc_attr( $field_key ) . '"> ' . esc_html( $field_val ) . '</label></li>';
				}
				
				$html = '<ul class="dreaming-wccp-fields-list dreaming-wccp-fields-attrs-list dreaming-wccp-sortable">' . $html . '</ul>';
			}
			
			$html .= '<input type="hidden" name="compare_fields_attrs" class="dreaming-wccp-field dreaming-wccp-field-hidden" value="' . esc_attr( implode( ',', $selected_fields ) ) . '" />';
			$html .= '<input type="hidden" name="all_compare_fields_attrs_order" class="dreaming-wccp-field dreaming-wccp-field-hidden" value="" />';
			
			return apply_filters( 'dreaming_admin_compare_fields_cb_html', $html, $fields );
		}
		
		public static function template_path() {
			return apply_filters( 'dreaming_wccp_template_path', 'dreaming-wccp/' );
		}
		
		public static function get_template_part( $slug, $name = '' ) {
			// Look in yourtheme/dreaming-wccp/slug-name.php and yourtheme/dreaming-wccp/slug.php
			$template = locate_template( array(
				                             self::template_path() . "{$slug}-{$name}.php",
				                             self::template_path() . "{$slug}.php"
			                             ) );
			
			$template_path = apply_filters( 'dreaming_wccp_set_template_path', MOORABI_TOOLKIT_PATH . 'includes/dreaming-woocommerce-compare//templates', $template );
			...
			$template = apply_filters( 'dreaming_wccp_get_template_part', $template, $slug, $name );
		...
		public static function get_template( $template_name, $args = array(), $template_path = '', $default_path = '' ) {
			...
			do_action( 'dreaming_wccp_before_template_part', $template_name, $template_path, $located, $args );
			
			include( $located );
			
			do_action( 'dreaming_wccp_after_template_part', $template_name, $template_path, $located, $args );
		}
		...
		public static function locate_template( $template_name, $template_path = '', $default_path = '' ) {
			...
			if ( ! $default_path ) {
				$default_path = MOORABI_TOOLKIT_PATH . 'includes/dreaming-woocommerce-compare/templates/';
			}
			...
			return apply_filters( 'dreaming_wcpc_locate_template', $template, $template_name, $template_path );
		}
		...
		public static function no_images(
			...
			$noimage_dir = MOORABI_TOOLKIT_PATH . 'includes/dreaming-woocommerce-compare/assets';
			$noimage_uri = MOORABI_TOOLKIT_URL . 'includes/dreaming-woocommerce-compare/assets';
		...
		public static function resize_image( $attach_id = null, $img_url = null, $width, $height, $crop = false, $place_hold = true, $use_real_img_hold = true, $solid_img_color = null ) {
			...
			$image_src = array();
			if ( $attach_id ) {
				if ( has_image_size( "dreaming_img_size_{$width}x{$height}" ) ) {
					$image_src = wp_get_attachment_image_src( $attach_id, "dreaming_img_size_{$width}x{$height}" );
					if ( ! empty( $image_src ) ) {
		...
		public static function clean( $var ) {
			if ( is_array( $var ) ) {
				return array_map( array( 'Dreaming_Woocompare_Helper', 'clean' ), $var );
			} else {
