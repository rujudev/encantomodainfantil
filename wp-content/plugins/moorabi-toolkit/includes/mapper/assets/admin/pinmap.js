(function ($) {
    ...
    var initTab = function () {
        $(document).on('click', '.moorabi-pinmap ul.nav li', function (e) {
    ...
    var imageSelector = function () {
        $(document).on('click', '.moorabi-pinmap a.btn-add-image, .moorabi-pinmap #change-image, .moorabi-pinmap a.image-selector', function (event) {
            ...
            if (!window.wr_image_selector) {
                ...
                window.wr_image_selector = wp.media({
                    button: {
                        text: moorabi_pinmap_params.text.img_selector_btn_label,
                    },
                    states: [new wp.media.controller.Library({
                        title   : moorabi_pinmap_params.text.img_selector_modal_title,
                ...
                window.wr_image_selector.on('select', function () {
                    ...
                    if (window.wr_image_selector.input_element.attr('id') == 'moorabi_pinmap_image') {
                        window.wr_image_selector.input_element.val(attachment.attributes.id);

                        // Update edit area with new image.
                        if ($('.moorabi-pinmap-bot > .edit-image').length) {
                            $('.moorabi-pinmap-bot .image-wrap > img').attr('src', attachment.attributes.url);
                        } else {
                            var edit_html = $('#moorabi_pinmap_image_tmpl').text().replace('%URL%', attachment.attributes.url);

                            $('.moorabi-pinmap-bot > .add-image').replaceWith(edit_html);
            ...
            window.wr_image_selector.input_element = $(this).hasClass('image-selector') ? $(this).prev('input') : $('#moorabi_pinmap_image');
    ...
    var setupApplication = function () {
        ...
        var PinList = Backbone.Collection.extend({
            ...
            fetch: function () {
                if (window.moorabi_pinmap_pins && window.moorabi_pinmap_pins.length) {
                    this.add(window.moorabi_pinmap_pins);
                }
            },
        });
        ...
        var PinView = Backbone.View.extend({
            tagName  : 'div',
            className: 'moorabi-pin',

            // Cache the template function for a single item.
            template: _.template($('#moorabi_pinmap_pin_tmpl').text()),
            ...
            render: function () {
                ...
                this.$el.find('[data-option]').each(function () {
                    ...
                    if ($(this).attr('name') != option[0]) {
                        if (option[3] !== undefined) {
                            $(this).attr('name', 'moorabi_pinmap_pins[' + self.index + '][' + option[1] + '][' + option[3] + ']');
                        } else {
                            $(this).attr('name', 'moorabi_pinmap_pins[' + self.index + '][' + option[1] + ']');
                        }
                    }
                    ...
                    switch ($(this).attr('data-option')) {
                        ...
                        case 'settings[popup-height]':
                            ...
                            $(this).attr('value', $('#general-settings [name*="' + setting[1] + '"]').val());
                            break;
                    ...
                    if ($(this).hasClass('color-picker')) {
                        $(this).moorabiColorPicker();

                        setTimeout($.proxy(function () {
                            $(this).parent().parent().click(function () {
                                // Hide all icon selector popup if not being
                                // focused.
                                $('.moorabi-pinmap .icon-selector').each(function () {
                    ...
                    // Init icon selector if needed.
                        ...
                        $(this).addClass('hidden').after($('#moorabi_pinmap_icon_selector_tmpl').text().replace('%SELECTED%', $(this).val()));
                    }
                    ...
                    // Init product selector if needed.
                        ...
                        $(this).select2({
                            ...
                            ajax              : {
                                url     : moorabi_pinmap_params.product_selector.url,
                                dataType: 'json',
                                delay   : 250,
                                data    : function (terms) {
                                    return {
                                        term    : terms,
                                        security: moorabi_pinmap_params.product_selector.security,
                                    };
                                },
                            ...
                            initSelection     : function (element, callback) {
                                ...
                                if (id !== '') {
                                    $.ajax(moorabi_pinmap_params.product_selector.url + '&term=' + id + '&security=' + moorabi_pinmap_params.product_selector.security, {
                                        dataType: 'json',
                                    }).done(function (data) {
                                        if (data !== null)
                                            callback({
                ...
                }).on('click', '.moorabi-icon-list a', function (event) {
                    event.preventDefault();
            ...
            edit: function (event) {
                if (!$(event.target).parent().hasClass('moorabi-pin') || this.just_dragged) {
                    return;
                }
                ...
                if (this.$el.hasClass('editing')) {
                    ...
                    if (!this.$el.data('moorabi_pinmap_pin_settings_initialized')) {
                        var self = this;
                        ...
                        this.$el.data('moorabi_pinmap_pin_settings_initialized', true);
                    }
            ...
            close: function (event) {
                ...
                if (pin_type == 'woocommerce') {
                    ...
                    this.$el.find('.tooltip').text(title ? title : moorabi_pinmap_params.text.please_input_a_title).removeClass('hidden');
                }
            ...
            remove: function (event) {
                ...
                if (confirm(moorabi_pinmap_params.text.confirm_removing_pin)) {
                    this.$el.remove();
                    this.model.destroy();
        ...
        var PinListView = Backbone.View.extend({
            ...
            el: '.moorabi-pinmap-bot > .edit-image > .image-wrap',
            ...
            initialize: function () {
                ...
                $(document).on('click', '.moorabi-pinmap-bot > .edit-image > .image-wrap > img', this.create);
        ...
        $(document).on('init_moorabi_pinmap', function (event) {
            if (!$(document).data('moorabi_pinmap_settings_initialized')) {
                ...
                $('#general-settings').find('input, select, textarea').each(function () {
                    if (window.moorabi_pinmap_settings[$(this).attr('name')]) {
                        $(this).val(window.moorabi_pinmap_settings[$(this).attr('name')]);

                        if ($(this).attr('type') == 'hidden' && $(this).next().attr('type') == 'checkbox') {
                            if (parseInt(window.moorabi_pinmap_settings[$(this).attr('name')])) {
                                $(this).next().attr('checked', 'checked');
                            } else {
                                $(this).next().removeAttr('checked');
                            }
                    ...
                    $(this).attr('name', 'moorabi_pinmap_settings[' + $(this).attr('name') + ']');
                ...
                $('#general-settings').on('change', 'input[type="radio"], select', function () {
                    var option = $(this).attr('name').match(/moorabi_pinmap_settings\[([^\]]+)\]/),
                        value  = $(this).val();
                ...
                $(document).click(function (event) {
                    ...
                    $('.moorabi-pinmap .wp-picker-holder .iris-picker').each(function () {
                    ...
                    $('#general-settings.editing, .moorabi-pin.editing').each(function () {
                ...
                // Track data changes.
                var data_fields = '.moorabi-pinmap input, .moorabi-pinmap select, .moorabi-pinmap textarea';

                $(document).on('change', data_fields, function () {
                    if ($(this).attr('name') && $(this).attr('name').indexOf('moorabi_pinmap_') > -1) {
                        $('#post #publish').attr('data-changed', 'yes');
                    }
                });
                ...
                $(window).on('beforeunload', function () {
                    ...
                    if (submit_btn.length && submit_btn.attr('data-changed') == 'yes') {
                        return moorabi_pinmap_params.text.ask_for_saving_changes;
                    }
                });

                $(document).data('moorabi_pinmap_settings_initialized', true);
            }

            if (!window.moorabi_pinmap_pins_app && $('.moorabi-pinmap-bot > .edit-image > .image-wrap').length) {
                // Init pin list view.
                window.moorabi_pinmap_pins_app = new PinListView;
            }
        });
    };
